...
dev() {
  ...
  if [ "$2" = "stop" ]; then
    warn "Stoping of tmux session: $pname"
    tmux kill-session -t "$pname" 2>/dev/null
    exit 0
  fi
...
deploy() {
  ...
  ssh "$host" "bash -s" -- "$dir" "$pname" <<'EOF'
        ...
        tmux send-keys -t "$REMOTE_PNAME:0" "unzip -qo web.zip && rm web.zip" C-m
        tmux send-keys -t "$REMOTE_PNAME:0" "chmod +x ./bin/backend && ./bin/backend" C-m
EOF
...
renewal() {
  ...
  echo -e "1. GLOBAL REPLACE: All occurrences of '$pname' will be changed to '$new_name'."
  ...
  if [[ "$confirm" =~ ^[yY](es)?$ ]]; then
    ...
    find . \
      ...
      -o -name "bin" \) -prune \
      -o -type f \
      -exec "${SED_CMD[@]}" "s/${pname}/${new_name}/g" {} +
    # generate docs
    docs
...
push() {
  commit_info=$2
  if [ -z "$2" ]; then
    warn "Missing commit info, use default commit info (upd)"
    commit_info=upd
  fi
  git add -A
