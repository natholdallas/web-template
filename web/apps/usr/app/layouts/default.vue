<script setup lang="ts">
import { Home, LogOut, Menu, Settings } from '@lucide/vue'
import { locales } from '~/lib/locale'

const auth = useAuth()
const isMobileOpen = ref(false)
const isLogoutDialogOpen = ref(false)

const conf = useConf()
const menuItems = computed(() => [
  { label: $t('urls.index'), icon: Home, to: '/' },
  { label: $t('urls.settings'), icon: Settings, to: '/settings' },
])
</script>

<template>
  <div class="flex fixed size-full overflow-hidden">
    <UiSheet v-model:open="isMobileOpen">
      <UiSheetContent side="left" class="w-72 flex flex-col h-full">
        <div class="flex items-center gap-2 px-4 py-4 border-b">
          <UiSheetTitle>{{ $t('app.name').toUpperCase() }}</UiSheetTitle>
          <UiSheetDescription>{{ $t('app.name') }}</UiSheetDescription>
        </div>
        <UiScrollArea class="flex-1 px-2">
          <nav class="space-y-1 py-2">
            <NuxtLink
              v-for="item in menuItems"
              :key="item.to"
              :to="item.to"
              @click="isMobileOpen = false"
              :class="[
                'flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors',
                $route.path === item.to
                  ? 'bg-primary text-primary-foreground'
                  : 'text-muted-foreground hover:text-foreground hover:bg-secondary',
              ]"
            >
              <component :is="item.icon" class="w-5 h-5" />
              <span>{{ item.label }}</span>
            </NuxtLink>
          </nav>
        </UiScrollArea>
      </UiSheetContent>
    </UiSheet>

    <div
      :class="[
        'hidden md:flex flex-col border-r bg-card transition-all duration-300 shrink-0',
        conf.isSidebarOpen ? 'w-64' : 'w-16',
      ]"
    >
      <div class="p-4 flex items-center justify-between border-b min-h-14">
        <span v-if="conf.isSidebarOpen" class="font-semibold text-lg">{{ $t('app.name').toUpperCase() }}</span>
        <UiButton variant="ghost" size="icon" @click="conf.isSidebarOpen = !conf.isSidebarOpen">
          <Menu class="w-4 h-4" />
        </UiButton>
      </div>

      <UiScrollArea class="flex-1 px-2">
        <nav class="space-y-1 py-2">
          <NuxtLink
            v-for="item in menuItems"
            :key="item.to"
            :to="item.to"
            :class="[
              'flex items-center gap-3 px-3 py-2 rounded-md text-sm font-medium transition-colors',
              $route.path === item.to
                ? 'bg-primary text-primary-foreground'
                : 'text-muted-foreground hover:text-foreground hover:bg-secondary',
            ]"
          >
            <component :is="item.icon" class="w-5 h-5 shrink-0" />
            <span v-if="conf.isSidebarOpen">{{ item.label }}</span>
          </NuxtLink>
        </nav>
      </UiScrollArea>
    </div>

    <div class="flex flex-col size-full overflow-hidden">
      <header class="h-15 border-b sticky top-0 flex items-center justify-between px-4 shrink-0">
        <div class="flex items-center gap-2">
          <UiButton variant="ghost" class="md:hidden" @click="isMobileOpen = true">
            <Menu class="w-5 h-5" />
          </UiButton>
          <span class="text-sm text-muted-foreground capitalize">{{ $t(`urls.${$route.name}`) }}</span>
        </div>

        <div class="flex items-center gap-1">
          <SwitchLang
            @update="$i18n.setLocale"
            :options="Object.values(locales).map(({ k, v }) => ({ label: $t(k), key: k, value: v }))"
            :value="$t(locales[$i18n.locale].k)"
          />
          <SwitchTheme />
          <UiButton variant="ghost" size="icon" @click="isLogoutDialogOpen = true">
            <LogOut class="w-5 h-5" />
          </UiButton>
        </div>
      </header>

      <main class="flex-1 min-h-0 overflow-hidden">
        <slot />
      </main>
      <div class="contents">
        <slot name="clean" />
      </div>
    </div>

    <UixModal
      v-model:open="isLogoutDialogOpen"
      :title="$t('sign.out')"
      :description="$t('sign.out.desc')"
      @cancel="isLogoutDialogOpen = false"
      @confirm="auth.$signOut"
    />
  </div>
</template>
