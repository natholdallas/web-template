...
- **Lint:** after writing frontend code, run lint in **every app** under `web/apps/` (adm, usr, and any others like `xxx`, `xxx2`). Each app has its own `lint`/`lint:fix` scripts (`eslint .` / `eslint . --fix`), so run `pnpm lint` (or `pnpm lint:fix` to autofix) from each app dir — there is no root-level lint script. Each app has its own `eslint.config.mjs` importing that app's generated `.nuxt/eslint.config.mjs` (adm additionally disables `vue/valid-v-slot` for Vuetify dotted slot names, and both apps ignore the generated `app/lib/sdk`); `.nuxt/` must exist first (`postinstall` runs `nuxt prepare`).
